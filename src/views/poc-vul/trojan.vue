<template>
    <div class="app-container">
        ...
        :data="list" border fit highlight-current-row style="width: 100%;" :default-sort = "{prop: 'date', order: 'descending'}">
            ...
            <el-table-column label="资产ID" prop="id" min-width="20px" align="center">
            ...
            <el-table-column label="Effect Server" min-width="30px" align="center">
                <template slot-scope="{row}">
                    <span>{{ row.asset_name }}:{{row.port_name}}</span>
                </template>
            </el-table-column>

            <el-table-column label="Real Server" min-width="30px" align="center">
                <template slot-scope="{row}">
                    <span>{{row.real_server}}</span>
                </template>
            </el-table-column>
            <el-table-column label="标签" min-width="30px" align="center">
                <template slot-scope="{row}">
                <div v-if="row.line==1"><el-tag>container</el-tag><el-tag>proxy</el-tag><el-tag>Lan</el-tag></div>
                <div v-else><el-tag>virtual-hardware</el-tag><el-tag>host</el-tag><el-tag>Wan</el-tag></div>
                </template>
            </el-table-column>
            <el-table-column label="状态" min-width="20px" align="center">
                <el-tag type="success">在线</el-tag>
            </el-table-column>
            <el-table-column label="上线时间" min-width="30px" align="center" prop="create_at" sortable>
                <template slot-scope="{row}">
                    <span>{{row.create_at}}</span>
                </template>
            </el-table-column>
            <el-table-column label="远程会话" min-width="40px" align="center">
                <template slot-scope="{row}">
                    <div style="margin-top: 15px;">
                        ...
                        <el-button size="mini" type="success" slot="append" icon="el-icon-phone-outline" @click="handlerWs(row)">Session会话</el-button>
                    </div>
                </template>
            </el-table-column>
            <el-table-column label="Operation" align="center" min-width="30px" class-name="small-padding fixed-width">
